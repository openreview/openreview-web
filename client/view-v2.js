  ...
  */
  ...
  var mkPdfSectionV2 = function(fieldDescription, fieldValue) {
    ...
    if (invitationFileTransfer === 'upload') {
      return mkFileRowV2(view.mkFileInput('pdf', 'file', order, regexStr), 'pdf', fieldDescription, fieldValue);

    } else if (invitationFileTransfer === 'url') {
      return mkFileRowV2(view.mkFileInput('pdf', 'text', order, regexStr), 'pdf', fieldDescription, fieldValue);
      ...
      return mkFileRowV2([$radioItem, $span], 'pdf', fieldDescription, fieldValue);
    }
  };
  ...
  var mkAttachmentSectionV2 = function(fieldName, fieldDescription, fieldValue) {
    ...
    if (invitationFileTransfer === 'upload') {
      return mkFileRowV2(view.mkFileInput(fieldName, 'file', order, regexStr), fieldName, fieldDescription, fieldValue);

    } else if (invitationFileTransfer === 'url') {
      return mkFileRowV2(view.mkFileInput(fieldName, 'text', order, regexStr), fieldName, fieldDescription, fieldValue);
  ...
  var getTitleTextV2 = function(note, generatedTitleText) {
  ...
  var mkPdfIconV2 = function(note, isReference) {
    // PDF for title
    var $pdfLink = null;
    if (note.content.pdf?.value) {
      var downloadURL = pdfUrlV2(note, isReference);
      $pdfLink = $('<a>', {
  ...
  var mkHtmlIconV2 = function(note) {
    var $htmlLink = null;
  ...
  var getAuthorTextV2 = function(note) {
  ...
  var buildContentV2 = function(note, params, additionalOmittedFields) {
    ...
    var contentOrder = invitation
      ? _.union(order(invitation.edit?.note?.content, invitation.id), contentKeys)
      : contentKeys;
      ...
      if (valueString.indexOf('/attachment/') === 0) {
        $contents.push($('<div>', {class: 'note_contents'}).append(
          ...
          $('<span>', {class: 'note_content_value'}).html(
            view.mkDownloadLink(note.id, fieldName, valueString, { isReference: params.isReference })
          )
        ));
        return;
      }
  ...
  var mkNotePanelV2 = function(note, options) {
    var params = _.assign({
      invitation: null,
    ...
    var titleText = getTitleTextV2(note, generatedTitleText);
    ...
    var $pdfLink = mkPdfIconV2(note, params.isReference);
    var $htmlLink = mkHtmlIconV2(note);
    ...
    var authorText = getAuthorTextV2(note);
    ...
    var $dateItem = (!notePastDue || details.writable) ?
      ...
      null;
    var $invItem = $('<span>', {class: 'item'}).text(options.isReference ? view.prettyId(note.invitations[0]) : note.content.venue?.value || view.prettyId(note.invitations[0]));
  ...
  var pdfUrlV2 = function(note, isReference) {
  ...
  var mkNoteEditorV2 = function(note, invitation, user, options) {
    var params = _.assign({
      onNoteCreated: null,
      onCompleted: null,
      onNoteCancelled: null,
