...
module.exports = function (forumId, noteId, invitationId, user, isBetaUser) {
        ...
        var replyInvitations = invitations
          .filter((p) => {
            ...
            return (
              replyTo &&
              (replyTo === note.id ||
                replyTo.param?.withForum === forumId ||
  ...
  var onNoteRecsChanged = function (noteRecs) {
    ...
    if (isBetaUser) {
      $('#beta-message')
        .html(
          '<p><span class="glyphicon glyphicon-exclamation-sign pr-1 " aria-hidden="true"></span> OpenReview is testing a new version of this page and needs your feedback. To try the new forum click here: <a href="/forum' +
  ...
  // Returns Notes that are parents (replyto) of the intersection Notes. This is done so that
  // the Note is expanded as well as its parents, otherwise, it would not be visible.
  ...
  // Takes an object that maps Invitatiions/Signatures to Notes and an array that contains all
  // the filters (key values) to map the Invitations/Signatures to their corresponding Note.
