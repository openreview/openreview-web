  ...
  var getNoteRecsP = function() {
    ...
    var noteRecsP = $.when(notesP, invitationsP).then(function (notes, invitations) {
      var noteRecPs = _.map(notes, function (note) {
        var deleteInvitation = invitations.filter(p => p.edit?.note?.id === note.id || note.invitations.includes(p.edit?.note?.id?.param?.withInvitation))
        ...
        var editInvitations = invitations.filter(p => p.edit?.note?.id === note.id || note.invitations.includes(p.edit?.note?.id?.param?.withInvitation))
        ...
        var replyInvitations = invitations
          .filter(p => {
            ...
            return replyTo && (replyTo === note.id || replyTo.param?.withForum === forumId || (replyTo.param?.withInvitation && note.invitations.includes(replyTo.param.withInvitation)))
          })
