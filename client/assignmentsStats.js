 ...
 * - added a function to set windowAssignmentConfigNote (window.assignmentConfigNote)
 * - changed some assignmentConfigNote names
 * - changed useEdges to be a function so it can get lastest value of windowAssignmentConfitNote
 * - added method runAssignmentsStats to chain method calls
**/
var Discrete = 'discrete';
var Continuous = 'continuous';
var Scalar = 'scalar';
var windowAssignmentConfigNote = null;
var useEdges = ()=>{return _.has(windowAssignmentConfigNote.content, 'scores_specification')}

var loadFromNotes = function(assignmentConfigNote) {
  return Webfield.getAll('/notes', {
    invitation: assignmentConfigNote.assignment_invitation,  // jshint ignore:line
    'content.title': windowAssignmentConfigNote.content.title
  }).then(function(notes) {
...
var loadFromEdges = function(assignmentConfigNote) {
  ...
  var assignmentsP = Webfield.getAll('/edges', {
    ...
    label: windowAssignmentConfigNote.content.title,
    groupBy: 'head',
    select: 'tail,weight'
  }, 'groupedEdges');
  ...
  bar.each(function(d, i) {
    ...
    if (stats.interactiveData) {
      ...
      hoverRect.on('click', function() {
        ...
        if (useEdges) {
          window.location.href = buildEdgeBrowserUrl(windowAssignmentConfigNote.content, key, stats.type);
        } else {
          window.location.href = '/assignments/browse?id=' + windowAssignmentConfigNote.id + '&showData=' + key;
        }
      });
    }
  });
...
var showParamsModal = function(e) {
  var paramsHtml = Handlebars.templates['partials/assignmentParameters']({
    parameters: windowAssignmentConfigNote.content
  });
  $(this).find('.modal-body').html(paramsHtml);
};


var buildEdgeBrowserUrl = function(configNote, key, type) {
...
if (_.isEmpty(windowAssignmentConfigNote) || windowAssignmentConfigNote.status !== 'Complete') {
  showLoadingError(
    'The requested paper assignment does not appear to have been run yet. ' +
    'Please contact info@openreview.net for more information.'
  );
}

const setWindowAssignmentConfigNote = (note) => {
  windowAssignmentConfigNote = note
}

const runAssignmentsStats = (note) => {
  loadMatchingData(note)
    .then(computeStats)
    .then(renderStatsMap)
