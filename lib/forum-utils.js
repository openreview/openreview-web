...
export function getNoteInvitations(invitations, note) {
  const deleteInvitation = invitations
    .filter(
      (p) =>
        p.edit?.note?.id?.const === note.id ||
        note.invitations.includes(p.edit?.note?.id?.withInvitation)
    )
  ...
  let editInvitations = invitations.filter(
    (p) =>
      p.edit?.note?.id?.const === note.id ||
      note.invitations.includes(p.edit?.note?.id?.withInvitation)
  )
