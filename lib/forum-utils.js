 ...
 * @returns object
 */
export function formatNote(note, invitation, editInvitations, deleteInvitation, replyInvitations) {
 ...
 * @returns array
 */
export function getNoteInvitations(invitations, note, forumId) {
  ...
  const replyInvitations = invitations.filter(p => {
    ...
    return p.details.repliesAvailable && replyTo && (
      replyTo.const === note.id || replyTo.withForum === forumId || (
