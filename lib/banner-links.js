 ...
 * @param {object} note
 */
export function forumLink(note, isVersion2Note = false) {
  const {
    forum, id, invitation, signatures, content,
  } = note
  const forumUrl = forum
    ? `/forum?id=${forum}${(id && id !== forum) ? `&noteId=${id}` : ''}`
    : '/'
  const title = (isVersion2Note ? content.title.value : content.title) || buildNoteTitle(invitation, signatures)
