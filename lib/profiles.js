...
function xpathSelect(xpathExpression, contextNode, namespace) {
  let namespaceResolver = null
  switch (namespace) {
    case 'atom':
      namespaceResolver = () => 'http://www.w3.org/2005/Atom'
      break
    case 'orcid':
...
export async function getOrcidPublicationsFromXmlUrl(orcid, profileNames) {
  const personUrl = `https://pub.orcid.org/v3.0/${orcid}/person`
  const worksUrl = `https://pub.orcid.org/v3.0/${orcid}/works`
  const familyNameSelector = '//personal-details:family-name/text()'
  const givenNameSelector = '//personal-details:given-names/text()'
  const otherNamesSelector = '//other-name:other-name/other-name:content/text()'
  const nonPersonalSourcePaperPutCodeSelector =
    '//work:work-summary[not(common:source/common:source-orcid)]/@put-code'
  const personXmlDoc = await $.ajax(personUrl)
  const familyName = xpathSelect(familyNameSelector, personXmlDoc, 'orcid')[0]?.textContent
  const givenName = xpathSelect(givenNameSelector, personXmlDoc, 'orcid')[0]?.textContent
  const otherNames = xpathSelect(otherNamesSelector, personXmlDoc, 'orcid').map(
    (name) => name.textContent
  )
  const allNames = otherNames.concat([`${givenName}${familyName ? ` ${familyName}` : ''}`])
  if (!profileNames.some((p) => allNames.includes(getNameString(p)))) {
    // user is using other people's orcid
    throw new Error('Please use your own ORCID')
  }
  const worksXmlDoc = await $.ajax(worksUrl)
  const publicationNodes = xpathSelect(
    nonPersonalSourcePaperPutCodeSelector,
    worksXmlDoc,
    'orcid'
  )
  if (!publicationNodes.length) {
    throw new Error('No publications to import')
  }
  const allPublicationIds = publicationNodes.map((p) => p.textContent).join(',')
  const publicationDetailsUrl = `https://pub.orcid.org/v3.0/${orcid}/works/${allPublicationIds}`
  const publicationDetailsXmlDoc = await $.ajax(publicationDetailsUrl)

  const workNodes = xpathSelect('//work:work', publicationDetailsXmlDoc, 'orcid')
  const publications = []
  workNodes.forEach((workNode) => {
    const title = workNode.getElementsByTagName('common:title')[0]?.textContent
    const abstract = workNode.getElementsByTagName('work:short-description')[0]?.textContent
    const authorNames = Array.from(workNode.getElementsByTagName('work:contributor')).map(
      (authorNode) => {
        const authorName = authorNode.getElementsByTagName('work:credit-name')[0]?.textContent
        return authorName
      }
    )
    const html = workNode.getElementsByTagName('common:url')[0]?.textContent
    const cdate = workNode.getElementsByTagName('common:created-date')[0]?.textContent
    const mdate = workNode.getElementsByTagName('common:last-modified-date')[0]?.textContent
    publications.push({
      title,
      ...
      authorIndex: authorNames.findIndex((name) => {
        // name may contain comma or have fist name last name order switched
        const nameTokens = name.split(',')
        let possibleNames = [deburrString(name, false)]
        if (nameTokens.length > 1) {
          const firstNameToken = nameTokens[0].trim()
          const lastNameToken = nameTokens[1].trim()
          possibleNames = possibleNames.concat([
            deburrString(`${firstNameToken} ${lastNameToken}`, false),
            deburrString(`${lastNameToken} ${firstNameToken}`, false),
          ])
        }
        return profileNames.some((p) =>
          possibleNames.includes(deburrString(getNameString(p), false))
        )
      }),
      authorCount: authorNames.length,
      venue: workNode.getElementsByTagName('common:source-name')[0]?.textContent,
      year: workNode.getElementsByTagName('common:year')[0]?.textContent,
    })
  })
  return publications
}
...
export async function getOrcidPublicationsFromJsonUrl(orcid, profileNames) {
  ...
  const familyName = personJson.name['family-name']?.value
  const givenName = personJson.name['given-names']?.value
  const otherNames = personJson['other-names']['other-name'].map((p) => p.content)
  const allNames = otherNames.concat([`${givenName}${familyName ? ` ${familyName}` : ''}`])
  if (!profileNames.some((p) => allNames.includes(getNameString(p)))) {
    // user is using other people's orcid
    throw new Error('Please use your own ORCID')
  ...
  const allPublicationIds = worksJson.group?.flatMap((p) => {
    if (p['work-summary']?.[0]?.source?.['source-orcid']) return [] // personal
    return p['work-summary']?.[0]?.['put-code']
  })

  if (!allPublicationIds.length) {
    throw new Error('No publications to import')
  }
  const publicationDetailsUrl = `https://pub.orcid.org/v3.0/${orcid}/works/${allPublicationIds.join(',')}`

  const publicationDetailsJson = await $.ajax({
  ...
  publicationDetailsJson.bulk
    ...
    .forEach((workNode) => {
      ...
      const arxivId = workNode['external-ids']?.['external-id']?.find((externalId) => {
        if (externalId['external-id-type'] !== 'arxiv') return false
        return externalId['external-id-value']
      })
      publications.push({
        key: nanoid(),
        title,
        json: workNode,
        externalId: `orcid:${workNode['put-code']}`,
        arxivId,
        formattedTitle: titleNameTransformation(title),
        abstract,
        authorNames: authorNames.map((p) => p.replace(/,/g, '')),
        html,
        cdate: dayjs(cdate).valueOf(),
        mdate: dayjs(mdate).valueOf(),
        authorIndex: authorNames.findIndex((name) => {
...
export async function getAllOrcidPapers(profileId, accessToken) {
  try {
    const result = await api
      .getAll(
        '/notes',
        {
          ...
          invitation: `${process.env.SUPER_USER}/Public_Article/-/ORCID_Record`,
        },
        { accessToken }
      )
      .then((notes) =>
        notes.map((note) => ({
          ...
          authorCount: note.content.authors?.value.length,
          venue: note.content.venue.value,
          externalId: note.externalIds.find((p) => p.startsWith('orcid:')),
        }))
      )
    return result
  } catch (error) {
...
export async function postOrUpdateOrcidPaper(
  profileId,
  profileNames,
  accessToken,
  publication
  ...
  const { notes: existingNotesInOpenReview } = await api.get(
    '/notes',
    {
  ...
  // check if paper has been arxiv imported
  if (publication.arxivId) {
    const { notes: existingArxivNotesInOpenReview } = await api.get(
      '/notes',
      {
        externalId: publication.arxivId,
      },
      { accessToken }
    )
    if (existingArxivNotesInOpenReview) {
      return api.post(
        '/notes/edits',
        {
          invitation: `${process.env.SUPER_USER}/Public_Article/-/Authorship_Claim`,
          signatures: [profileId],
          note: {
            id: existingArxivNotesInOpenReview.id,
          },
          content: {
            author_index: {
              value: publication.authorIndex,
