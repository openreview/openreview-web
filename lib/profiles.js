...
export function formatProfileData(profileData, options = {}) {
  ...
  return Object.assign(_.pick(profileData.content, profileFields), {
    ...
    dob: useLinkObjectFormat ? { value: profileData.content.dob } : profileData.content.dob,
...
/**
 * Moderate a profile and record the decision as a Profile_State profile edit,
 * with the state the moderation endpoint returned and the reason when given.
 * The record is informative only and best effort: deployments without the
 * Profile_State invitation, and decisions that remove the subject (delete),
 * still moderate normally.
 *
 * @param {string} profileId - id of the profile to moderate
 * @param {string} decision - accept, reject, block, unblock, delete, restore or limit
 * @param {string} reason - optional reason for the decision; emailed to the user when rejecting
 * @param {string[]} labels - optional short labels of the selected reasons, stored for compact display
 */
export async function moderateProfile(profileId, decision, reason, labels) {
  const profile = await api.post('/profile/moderate', {
    id: profileId,
    decision,
    ...(reason ? { reason } : {}),
  })
  ...
  const supportGroupId = `${process.env.SUPER_USER}/Support`
  try {
    await api.post('/profiles/edits', {
      invitation: `${supportGroupId}/-/Profile_State`,
      signatures: [supportGroupId],
      content: {
        state: { value: profile.state },
        ...(reason ? { reason: { value: reason } } : {}),
        ...(labels?.length ? { labels: { value: labels } } : {}),
      },
...
/**
 * Load the rejection reasons offered by the moderation UI from the labels enum of the
 * Profile_State invitation edit schema. The enum is the controlled vocabulary of
 * moderation labels (the schema rejects any other label on a Profile_State edit), and
 * an entry's description is the default rejection message for that label; entries
 * without a description (labels used outside the rejection flow) are not offered.
 * The support team manages the vocabulary by editing the invitation.
 * {{issuedByCurrentInstitution}} in a default message is replaced with
 * "issued by <currentInstitutionName> " when an institution is given, and removed
 * otherwise. Falls back to the hardcoded list when the invitation does not exist or
 * does not declare the enum.
 *
 * @param {string} currentInstitutionName - name of the profile's current institution, if any
 */
const getProfileStateLabelOptions = async () => {
  const { invitations } = await api.get('/invitations', {
    id: `${process.env.SUPER_USER}/Support/-/Profile_State`,
  })
  return invitations?.[0]?.edit?.content?.labels?.value?.param?.enum
}

export async function getModerationRejectionReasons(currentInstitutionName) {
  try {
    const labelOptions = await getProfileStateLabelOptions()
    const reasons = (labelOptions ?? []).filter((option) => option.value && option.description)
    if (!reasons.length) {
      return getRejectionReasons(currentInstitutionName)
    }

    const issuedBy = currentInstitutionName ? `issued by ${currentInstitutionName} ` : ''
    return reasons.map((option) => ({
      value: option.value,
      label: option.value,
      rejectionText: option.description.replaceAll('{{issuedByCurrentInstitution}}', issuedBy),
    }))
  } catch (error) {
    return getRejectionReasons(currentInstitutionName)
  }
...
/**
 * Load the labels the block modal offers: the entries of the Profile_State labels enum
 * that carry no description (labels used outside the rejection flow). Falls back to a
 * hardcoded pair when the invitation does not exist or declares none.
 */
export async function getModerationBlockLabels() {
  try {
    const labelOptions = await getProfileStateLabelOptions()
  ...
  return ['Suspicious Activity', 'Terms of Service Violation']
}

/**
 * Load the verification records for a profile: every profile edit readable by the
 * caller that asserts profile content (names, dob, history entries, relations).
 * Each asserted item carries the id of the entity that signed the verification —
 * today the support team, in the future vouchers or third party identity providers.
 * Returns null when the deployment has no profile edits support.
 *
 * @param {string} profileId - canonical tilde id of the profile
 * @param {object} options - api options ({ accessToken, remoteIpAddress }) for server-side calls
 */
export async function getProfileVerifications(profileId, options = {}) {
  try {
    const { edits } = await api.get(
      '/profiles/edits',
      { 'profile.id': profileId, limit: 100, sort: 'tcdate:asc' },
      options
    )

    const verifications = { names: [], dob: [], history: [], relations: [] }
    // dob is a scalar: a verifier asserting a new value supersedes their earlier
    // assertion, so only the latest record per verifier counts. Set-like fields
    // (names, history, relations) accumulate instead: verifying several is coherent.
    const dobByVerifier = new Map()
    ;(edits ?? []).forEach((edit) => {
      const content = edit.profile?.content
      const verifiedBy = edit.signatures?.[0]
      if (!content || !verifiedBy) return

      const addedItems = (field) => {
        const value = content[field]?.value
        return Array.isArray(value?.add) ? value.add : []
      }

      addedItems('names').forEach((name) => verifications.names.push({ ...name, verifiedBy }))
      addedItems('history').forEach((history) =>
        verifications.history.push({ ...history, verifiedBy })
      )
      addedItems('relations').forEach((relation) =>
        verifications.relations.push({ ...relation, verifiedBy })
      )
      if (content.dob?.value !== undefined) {
        dobByVerifier.set(verifiedBy, content.dob.value)
      }
    })
    verifications.dob = [...dobByVerifier.entries()].map(([verifiedBy, value]) => ({
      value,
      verifiedBy,
    }))
...
async function searchPublicationTitle(
  title,
  authorIndex,
  authorNames,
  venue,
  dblpPublicationExternalId
) {
  ...
  for (let index = 0; index < paperSearchResults.length; index += 1) {
    ...
    const existingNoteDblpExternalId = note.externalIds?.find((id) => id.startsWith('dblp:'))
    if (existingNoteDblpExternalId && typeof note.content?.authors?.value?.[0] === 'object') {
      // openreview paper is object author schema
      if (existingNoteDblpExternalId === dblpPublicationExternalId) {
        return {
          authorIndex: note.content.authors.value.findIndex((author) =>
            authorNames
              .map((authorName) => getNameString(authorName))
    ...
    if (noteTitle && titleNameTransformation(noteTitle) === title) {
      ...
      const noteAuthorIds = getNoteAuthorIds(note, note.version === 2)
      const noteAuthors = getNoteAuthors(note, note.version === 2)
    ...
    const v2NotesWithObjectAuthorSchema = await api
      ...
      .then((notes) =>
        notes.map((note) => ({
          ...
          authorCount: note.content.authors?.value.length,
          venue: note.content.venue.value,
          externalId: note.externalIds.find((p) => p.startsWith('dblp:')),
        }))
      )
    return v1Notes.concat(v2Notes, v2NotesWithObjectAuthorSchema)
  } catch (error) {
...
export async function postOrUpdatePaper(dblpPublication, profileId, names) {
  ...
  const publicationTitleExistInOpenReview = await searchPublicationTitle(
    ...
    dblpPublication.externalId
  )
  if (publicationTitleExistInOpenReview.dblpPublicationExternalId) {
    const authorId =
      names?.find((p) => dblpPublication.authorNames?.includes(getNameString(p)))?.username ??
      profileId
    return api.post('/notes/edits', {
      invitation: `${process.env.SUPER_USER}/Public_Article/-/Authorship_Claim`,
      signatures: [profileId],
      note: {
        id: publicationTitleExistInOpenReview.paperId,
      },
      content: {
        author_index: {
          value: publicationTitleExistInOpenReview.authorIndex,
        },
        author_id: {
          value: authorId,
        },
        author_name: {
          value: prettyId(authorId),
        },
      },
    })
  }
  ...
  const preferedNameObj = names?.find((p) => p.preferred) ?? names?.[0]
  const preferredName = preferedNameObj?.fullname

  const authors = dblpPublication.authorNames.map((name, index) => ({
    fullname: index === dblpPublication.authorIndex ? preferredName : name,
    username: index === dblpPublication.authorIndex ? tileIdToUpdate : undefined,
  }))
  ...
  return api.post('/notes/edits', {
    ...
    note: {
      ...
      externalId: dblpPublication.externalId,
    },
  })
}
...
export async function getAllPapersImportedByOtherProfiles(
  ...
  setESErrorBanner(searchResultByYear.some((result) => result.searchUnavailable))
  ...
  return dblpPublications.flatMap((publication, i) => {
    for (let index = 0; index < paperSearchResults.length; index += 1) {
      ...
      if (
        ...
        note.content?.authors?.value?.length === publication.authorCount &&
        note.content?.venue?.value === publication.venue
      ) {
        const authorId = note.content.authors.value[publication.authorIndex].username
        if (
          authorId &&
          authorId.startsWith('~') &&
    ...
    .forEach((workNode) => {
      ...
      let authorIndex = authorNames.findIndex((name) => {
        ...
        return profileNames.some((p) => {
          return normalizedName === normalizeName(p)
        })
      })
      if (authorIndex === -1)
...
export async function postOrUpdateOrcidPaper(profileId, profileNames, publication) {
  ...
  // new paper
  const profileNameObj = profileNames?.find((p) =>
    publication.authorNames?.includes(getNameString(p))
  )
  const authors = publication.authorNames.map((publicationAuthorName, index) => {
    const username = profileNameObj?.username ?? profileId
    const fullname = profileNameObj?.fullname ?? prettyId(username)

    return {
      fullname: index === publication.authorIndex ? fullname : publicationAuthorName,
      username: index === publication.authorIndex ? username : '',
    }
  })
  return api.post('/notes/edits', {
    invitation: `${process.env.SUPER_USER}/Public_Article/ORCID.org/-/Record`,
