...
import { apiV2MergeNotes, convertNotesToV2, convertSingleNoteToV2 } from './utils'
 ...
 */
export function formatProfileData(profileData, ignoreEmptyNameToken = false) {
  ...
  profileData.content.names.forEach((n, i) => {
    ...
    n.altUsernames = []
    const fullName = [n.first, n.middle, n.last].filter(p => (ignoreEmptyNameToken ? p : true)).join(' ')
...
export function getCoAuthorsFromPublications(profile, notes) {
  ...
  notes.forEach((note) => {
    ...
    const noteAuthors = process.env.ENABLE_V2_API
      ? note.content.authors.value.map(a => a.replace(/\*|\d$/g, ''))
      : note.content.authors.map(a => a.replace(/\*|\d$/g, ''))

    for (let i = 0; i < noteAuthors.length; i += 1) {
      const authorName = noteAuthors[i]
      allAuthors.add(authorName)

      const authorId = process.env.ENABLE_V2_API
        ? note.content.authorids.value && note.content.authorids.value[i]
        : note.content.authorids && note.content.authorids[i]
      if (authorId) {
        if ( // ~>email>dblp
          authorId.startsWith('~')
          || (authorId.includes('@') && (!authorNameToId[authorName] || authorNameToId[authorName].startsWith('http')))
...
function addAuthorToPaper(authorIds, authorIndex, paperId, authorGroupId, referenceId, accessToken, noteVersion) {
  ...
  if (process.env.ENABLE_V2_API && noteVersion === 'v2') {
    ...
    return api.postV2('/notes/edit', updateNoteObject, { accessToken })
  }
  const updateNoteObject = {
    id: referenceId,
    referent: paperId,
...
async function searchPublicationTitle(title, authorIndex, authorNames, accessToken) {
  ...
  if (process.env.ENABLE_V2_API) {
    const v1NotesP = api.get('/notes/search', queryParam, { accessToken })
    const v2NotesP = api.getV2('/notes/search', queryParam, { accessToken })
    const result = await Promise.all([v1NotesP, v2NotesP])
    const notes = apiV2MergeNotes(result[0].notes, result[1].notes)
    searchResults = {
      notes,
      count: notes.length,
    }
  } else {
    const result = await api.get('/notes/search', queryParam, { accessToken })
    searchResults = {
  ...
  for (let index = 0; index < searchResults.notes.length; index += 1) {
    ...
    if (note.content && note.content.title?.value && titleNameTransformation(note.content.title.value) === title) {
      // even if titles match, need to check if authorids field already contains
      // author id at authorIndex
      if (note.content.authorids?.value && note.content.authorids.value.length > authorIndex && (!note.content.authorids.value[authorIndex] || note.content.authorids.value[authorIndex].startsWith('https://dblp.org'))) {
        return {
          paperExistInOpenReview: true,
          authorNameInAuthorsList: note.content.authors.value.some(name => authorNames.includes(name)),
          paperId: note.forum,
...
function xpathSelect(xpathExpression, contextNode) {
  if (!document.evaluate) {
    ...
    const wgxpath = require('../client/wgxpath.install.js')
    wgxpath.wgxpath.install(window) // add document.evaluate support for ie11
  }
...
export async function getDblpPublicationsFromXmlUrl(xmlUrl, profileId) {
  ...
  try {
    ...
    return {
      notes: publicationNodes.filter(publicationNode => publicationNode.getElementsByTagName('author').length > 0).map((publicationNode) => {
        ...
        return {
          note: {
            content: {
              // call publicationNode.outerHTML when droping ie11 support
              dblp: serializer.serializeToString(publicationNode),
            },
            invitation: 'dblp.org/-/record',
            readers: ['everyone'],
            writers: ['dblp.org'],
...
export async function getAllPapersByGroupId(profileId, accessToken) {
  try {
    let notesList
    const queryParam = { 'content.authorids': profileId }
    const options = {
      accessToken,
      cachePolicy: 'no-cache',
    }
    if (process.env.ENABLE_V2_API) {
      const v1NotesP = api.get('/notes', queryParam, options)
      const v2NotesP = api.getV2('/notes', queryParam, options)
      const results = await Promise.all([v1NotesP, v2NotesP])
      notesList = apiV2MergeNotes(results[0].notes, results[1].notes)
    } else {
      ({ notes: notesList } = await api.get('/notes', queryParam, options))
    }
    return notesList.map(note => ({
      id: note.id,
      title: titleNameTransformation(process.env.ENABLE_V2_API ? note.content.title.value : note.content.title),
      authorCount: process.env.ENABLE_V2_API ? note.content.authors?.value?.length : note.content.authors?.length,
    }))
  } catch (error) {
...
export async function postOrUpdatePaper(dblpPublication, profileId, names, accessToken) {
  ...
  if (publicationTitleExistInOpenReview.paperExistInOpenReview
    ...
    return addAuthorToPaper(
      ...
      undefined,
      accessToken,
      publicationTitleExistInOpenReview.version,
    )
  }
  ...
  let dblpPaperNote
  if (process.env.ENABLE_V2_API) {
    dblpPaperNote = await api.postV2('/notes/edit', {
      id: null,
      invitation: dblpPublication.note.invitation,
      readers: dblpPublication.note.readers,
      writers: dblpPublication.note.writers,
      signatures: dblpPublication.note.signatures,
      note: convertSingleNoteToV2(dblpPublication.note),
    }, { accessToken })
  } else {
    dblpPaperNote = await api.post('/notes', dblpPublication.note, { accessToken })
  }
  return addAuthorToPaper(
    ...
    undefined,
    accessToken,
    process.env.ENABLE_V2_API ? 'v2' : 'v1',
  )
}
...
export async function getAllPapersImportedByOtherProfiles(dblpPublications, profileId, accessToken) {
  return Promise.all(dblpPublications.map(async (publication) => {
    ...
    if (process.env.ENABLE_V2_API) {
      const v1NotesSearchP = api.get('/notes/search', queryParam, { accessToken })
      const v2NotesSearchP = api.getV2('/notes/search', queryParam, { accessToken })
      const results = await Promise.all([v1NotesSearchP, v2NotesSearchP])
      notes = apiV2MergeNotes(results[0].notes, results[1].notes)
    } else {
      const notesInV1 = await api.get('/notes/search', queryParam, { accessToken })
      notes = convertNotesToV2(notesInV1.notes)
    }

    for (let index = 0; index < notes.length; index += 1) {
      const note = notes[index]
      if (note.content?.title?.value && titleNameTransformation(note.content.title.value) === publication.title
        ...
        if (authorId && authorId.startsWith('~') && authorId !== profileId) {
