...
/**
 * Moderate a profile. The moderation endpoint changes the profile state and records
 * the decision as a Profile_State profile edit, so no edit is posted here.
 *
 ...
 */
export function moderateProfile(profileId, decision, reason, labels) {
 ...
 * caller that asserts profile data. A verification edit asserts flat values read off
 * a document — a fullname, a dob, a single history entry, a single relation — as the
 * verification invitations declare them. Each asserted item carries the id of the
 * entity that signed the verification — today the support team, in the future
 * vouchers or third party identity providers. Returns null when the deployment has
 * no profile edits support.
 *
...
export async function getProfileVerifications(profileId, options = {}) {
  try {
    ...
    // assertion, so only the latest record per verifier counts. The other fields
    // accumulate instead: verifying several names, affiliations or relations is
    // coherent.
    ...
    ;(edits ?? []).forEach((edit) => {
      ...
      // The document the data was read off, e.g. Passport or Student ID.
      const source = edit.content?.source?.value

      if (content.fullname?.value) {
        verifications.names.push({ fullname: content.fullname.value, verifiedBy, source })
      }
      if (content.history?.value) {
        verifications.history.push({ ...content.history.value, verifiedBy, source })
      }
      if (content.relations?.value) {
        verifications.relations.push({ ...content.relations.value, verifiedBy, source })
      }
      if (content.dob?.value !== undefined) {
        dobByVerifier.set(verifiedBy, { value: content.dob.value, source })
      }
    })
    verifications.dob = [...dobByVerifier.entries()].map(([verifiedBy, { value, source }]) => ({
      value,
      verifiedBy,
      source,
    }))
