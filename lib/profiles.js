...
export async function getAllPapersImportedByOtherProfiles(
  ...
  const getBatchSearchResult = async (start, end) => {
    if (end - start > 1000)
      // terms param can take at most 1000 terms
      return [
        ...(await getBatchSearchResult(start, start + Math.floor((end - start) / 2))),
        ...(await getBatchSearchResult(start + Math.floor((end - start) / 2), end)),
      ]
    const publicationsToSearch = dblpPublications.slice(start, end)
    if (!publicationsToSearch.length) return []
    const batchResult = await api.post(
      '/notes/search',
      {
        terms: publicationsToSearch.map((p) => `"${p.title}"`),
        invitation: 'dblp.org/-/record',
        content: 'title',
      },
      {
    ...
    // api will return at most 1000 notes or not dividable anymore
    if (batchResult.count < 1000 || publicationsToSearch.length === 1) return batchResult.notes
    return [
      ...(await getBatchSearchResult(start, start + Math.floor((end - start) / 2))),
      ...(await getBatchSearchResult(start + Math.floor((end - start) / 2), end)),
    ]
  ...
  const searchResult = await getBatchSearchResult(0, dblpPublications.length)

  return dblpPublications.flatMap((publication, i) => {
    for (let index = 0; index < searchResult.length; index += 1) {
      const note = searchResult[index]
      if (
        note.content?.title &&
