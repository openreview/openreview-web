...
export async function getOrcidPublicationsFromJsonUrl(orcid, profileNames) {
  ...
  const uniquePublicationIds = [...new Set(allPublicationIds)]

  if (!uniquePublicationIds.length) {
    throw new Error('No publications to import')
  }

  const batchSize = 100 // max 100 allowed by ORCID
  const batchCount = Math.ceil(uniquePublicationIds.length / batchSize)

  const batchIds = Array.from(new Array(batchCount), (_e, index) => {
    const idsInBatch = uniquePublicationIds
