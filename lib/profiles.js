...
export function formatProfileLinks(profileContent, profileMeta, ids, useLinkObjectFormat) {
  ...
  if (useLinkObjectFormat) {
    return Object.keys(profileLinkMap).reduce((acc, linkType) => {
      ...
      if (linkContent) {
        acc[linkType] = { value: linkContent.toLowerCase().startsWith('http') ? linkContent : `http://${linkContent}` }
      }
      return acc
    }, {})
  }
 ...
 */
export function formatProfileData(profileData, ignoreEmptyNameToken = false, useLinkObjectFormat = false) {
  ...
  profileData.content.links = formatProfileLinks(profileData.content, profileData.metaContent, ids, useLinkObjectFormat)
...
async function searchPublicationTitle(title, authorIndex, authorNames, venue, accessToken) {
  ...
  for (let index = 0; index < searchResults.notes.length; index += 1) {
    ...
    if (note.content && note.content.title && titleNameTransformation(note.content.title) === title) {
      ...
      if (note.content.authorids && note.content.authorids.length > authorIndex
        && (!note.content.authorids[authorIndex] || note.content.authorids[authorIndex].startsWith('https://dblp.org'))
...
function xpathSelect(xpathExpression, contextNode) {
  if (!document.evaluate) {
    ...
    const wgxpath = require('../client/wgxpath.install')
  ...
  try {
    ...
    return {
      notes: publicationNodes.filter(publicationNode => publicationNode.getElementsByTagName('author').length > 0).map((publicationNode) => {
        ...
        let venue = publicationNode.getElementsByTagName('journal')[0]?.textContent ?? publicationNode.getElementsByTagName('booktitle')[0]?.textContent
...
export async function postOrUpdatePaper(dblpPublication, profileId, names, accessToken) {
  const publicationTitleExistInOpenReview = await searchPublicationTitle(
    dblpPublication.formattedTitle, dblpPublication.authorIndex, names, dblpPublication.venue, accessToken,
  )
...
export async function getAllPapersImportedByOtherProfiles(dblpPublications, profileId, accessToken) {
  return Promise.all(dblpPublications.map(async (publication) => {
    ...
    for (let index = 0; index < searchResult.notes.length; index += 1) {
      ...
      if (note.content?.title && titleNameTransformation(note.content.title) === publication.title
