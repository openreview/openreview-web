...
import { deburrString, getNameString, getNoteAuthorIds, getNoteAuthors } from './utils'
...
export async function postOrUpdateOrcidPaper(profileId, profileNames, publication) {
  ...
  // new paper
  const authors = publication.authorNames.map((name, index) => ({
    fullname: name,
    username:
      index === publication.authorIndex
        ? (profileNames?.find((p) => publication.authorNames?.includes(getNameString(p)))
            ?.username ?? profileId)
        : '',
  }))
