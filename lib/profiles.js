...
export async function getAllPapersByGroupId(profileId, accessToken) {
  try {
    ...
    const notesList = await api.get('notes/search', {
      term: profileId,
      content: 'authors',
    }, {
      accessToken,
...
export async function postOrUpdatePaper(dblpPublication, profileId, names, accessToken) {
  const publicationTitleExistInOpenReview = await searchPublicationTitle(dblpPublication.formattedTitle, dblpPublication.authorIndex, names, accessToken)
  if (publicationTitleExistInOpenReview.paperExistInOpenReview && publicationTitleExistInOpenReview.authorNameInAuthorsList) {
    ...
    return addAuthorToPaper(publicationTitleExistInOpenReview.authorIds, dblpPublication.authorIndex, publicationTitleExistInOpenReview.paperId, profileId, undefined, accessToken)
  }
  ...
  return addAuthorToPaper(dblpPaperNote.content.authorids, dblpPublication.authorIndex, dblpPaperNote.id, profileId, undefined, accessToken)
}
