...
async function searchPublicationTitle(title, authorIndex, authorNames, accessToken) {
  ...
  if (process.env.ENABLE_V2_API) {
    const v1NotesP = api.get('/notes/search', {
      term: title,
      invitation: 'dblp.org/-/record',
    }, {
      accessToken,
    })
    const v2NotesP = api.getV2('/notes/search', {
      term: title,
      invitation: 'dblp.org/-/record',
    }, {
      accessToken,
...
export async function getAllPapersByGroupId(profileId, accessToken) {
  try {
    ...
    if (process.env.ENABLE_V2_API) {
      ...
      const v2NotesP = api.getV2('/notes', {
        'content.authorids': profileId,
      }, {
        accessToken,
        cachePolicy: 'no-cache',
      ...
      ({ notes: notesList } = await api.get('/notes', {
        'content.authorids': profileId,
      }, {
        accessToken,
        cachePolicy: 'no-cache',
...
export async function getAllPapersImportedByOtherProfiles(dblpPublications, profileId, accessToken) {
  return Promise.all(dblpPublications.map(async (publication) => {
    let notes
    if (process.env.ENABLE_V2_API) {
      const v1NotesSearchP = api.get('/notes/search', {
        term: publication.title,
        invitation: 'dblp.org/-/record',
      }, {
        accessToken,
      })
      const v2NotesSearchP = api.getV2('/notes/search', {
        term: publication.title,
        invitation: 'dblp.org/-/record',
      }, {
        accessToken,
      ...
      const notesInV1 = await api.get('/notes/search', {
        term: publication.title,
        invitation: 'dblp.org/-/record',
      }, {
        accessToken,
