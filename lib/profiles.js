...
export async function getAllPapersByGroupId(profileId) {
  try {
    const notesList = await Webfield.get('notes/search', {
      term: profileId,
      content: 'authors',
    }, { cache: false }) // the data is updated anyway after import
...
async function postOrUpdatePaper(dblpPublication, profileId, names) {
  const publicationTitleExistInOpenReview = await searchPublicationTitle(dblpPublication.formattedTitle, dblpPublication.authorIndex, names)
  if (publicationTitleExistInOpenReview.paperExistInOpenReview && publicationTitleExistInOpenReview.authorNameInAuthorsList) {
    ...
    return addAuthorToPaper(publicationTitleExistInOpenReview.authorIds, dblpPublication.authorIndex, publicationTitleExistInOpenReview.paperId, profileId, undefined)
  }
  ...
  return addAuthorToPaper(dblpPaperNote.content.authorids, dblpPublication.authorIndex, dblpPaperNote.id, profileId, undefined)
}

function addAuthorToPaper(authorIds, authorIndex, paperId, authorGroupId, referenceId) {
...
async function searchPublicationTitle(title, authorIndex, authorNames) {
  const searchResults = await Webfield.get('notes/search', {
    term: title,
    invitation: 'dblp.org/-/record',
  })
...
function xpathSelect(xpathExpression, contextNode) {
  ...
  const resultIterator = document.evaluate(xpathExpression, contextNode, null, XPathResult.ORDERED_NODE_ITERATOR_TYPE)
  const xmlNodes = []
