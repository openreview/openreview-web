...
export async function getAllPapersImportedByOtherProfiles(
  ...
  const batchSize = process.env.DBLP_BATCH_SIZE ?? 1
  const sliceIndexes = [...Array(Math.ceil(dblpPublications.length / batchSize)).keys()].map(
    (p) => [p * batchSize, p * batchSize + batchSize]
  )
  const allSearchResult = await Promise.all(
    sliceIndexes.map(async ([start, end]) => {
      const combinedTitles = dblpPublications
        .slice(start, end)
        .reduce(
          (acc, curr) => (acc === '' ? `"${curr.title}"` : `${acc} OR "${curr.title}"`),
          ''
        )
      const batchSearchResult = await api.get(
        '/notes/search',
        {
          query: combinedTitles,
          invitation: 'dblp.org/-/record',
        },
        {
          accessToken,
      ...
      return batchSearchResult.notes
    })
  )
  return dblpPublications.flatMap((publication, i) => {
    const batchIndex = Math.floor(i / batchSize)
    const searchResult = allSearchResult[batchIndex]
