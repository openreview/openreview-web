...
export async function getProfileByDblpUrl(dblpUrl) {
  try {
...
export async function getAllPapersImportedByOtherProfiles(dblpPublications, profileId, accessToken) {
  const dblpPublicationsOfOtherProfiles = []
  return Promise.all(dblpPublications.map(async (publication) => {
    ...
    for (let index = 0; index < searchResult.notes.length; index += 1) {
      ...
      if (note.content && note.content.title && titleNameTransformation(note.content.title) === publication.title) {
        if (note.content.authorids && note.content.authorids.length > publication.authorIndex && note.content.authorids[publication.authorIndex].startsWith('~') && note.content.authorids[publication.authorIndex] !== profileId) {
          return { ...publication, existingProfileId: note.content.authorids[publication.authorIndex], noteId: note.id }
        }
      }
    }
    return null
