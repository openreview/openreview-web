...
const getInvitationById = async (invitationId, accessToken, queryParam1, queryParam2) => {
  const invitationObj = await getById('invitations', invitationId, accessToken, 2, queryParam1)
  if (invitationObj) {
    return invitationObj
  }

  return getById('invitations', invitationId, accessToken, 1, queryParam2 ?? queryParam1)
}

const getNoteById = async (noteId, accessToken, queryParam1, queryParam2) => {
  const noteObj = await getById('notes', noteId, accessToken, 2, queryParam1)
  if (noteObj) {
    return noteObj
  }

  return getById('notes', noteId, accessToken, 1, queryParam1 ?? queryParam2)
}

const getGroupById = (groupId, accessToken, queryParam) => getById('groups', groupId, accessToken, 2, queryParam)
...
const getCombined = async (path, data1 = {}, data2 = null, options = {}) => {
  ...
  if (!process.env.API_V2_URL) {
    const results = await get(path, data1, { ...options, version: 1 })
