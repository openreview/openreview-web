...
const getAll = async (path, queryParam, options = {}) => {
  ...
  let rest = []
  let lastId = initialResults.length > 0 ? initialResults[initialResults.length - 1].id : undefined
  for (const p of offsetList) {
    const hasAfter = Boolean(lastId)
    const reqParams = {
      ...queryObj,
      offset: p,
      ...(options.version !== 1 && { count: false }),
      ...(hasAfter && { after: lastId, with_count: false })
    }
    const res = await get(
      path,
      reqParams,
