...
const getAll = async (path, queryParam, accessToken, resultsKeyParam) => {
  ...
  const results = await get(path, queryObj, { accessToken })
  ...
  // const offsetList = _.range(offset + queryObj.limit, totalCount, queryObj.limit)
  const offsetList = [...Array(totalCount - offset).keys()]
    .map(p => p + offset)
    .filter(q => (q - offset) % queryObj.limit === 0)
  const remainingRequests = offsetList.map(p => get(path, { ...queryObj, offset: p }, { accessToken }))
