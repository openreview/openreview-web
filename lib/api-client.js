...
const formatError = (err) => {
  ...
  if (err instanceof TypeError) {
    networkErr = new Error('The OpenReview API is currently unavailable')
    networkErr.name = 'UnavailableError'
    networkErr.status = 500
    networkErr.details = 'Please wait and try again later.'
  }
...
const checkStatus = async (response) => {
  ...
  error.details = errorBody.errors?.length > 0 ? errorBody.errors[0] : errorBody.details
  error.legacy = Array.isArray(errorBody.errors)
...
const getById = async (apiPath, id, accessToken, apiVersion, queryParam) => {
  ...
  try {
    const apiRes = await get(`/${apiPath}`, { id, ...queryParam }, { accessToken, version: apiVersion })
...
const getCombined = async (path, data1 = {}, data2 = null, options = {}) => {
  ...
  return {
    [resultsKey]: options.noSort ? results2.concat(results1) : results1.concat(results2).sort(sortFn),
...
const getAll = async (path, queryParam, options = {}) => {
  ...
  const results = await get(path, queryObj, { accessToken: options.accessToken, version: options.version })
  ...
  const remainingRequests = offsetList.map(p => get(path, { ...queryObj, offset: p }, { accessToken: options.accessToken }))
  const remainingResults = await Promise.all(remainingRequests)
  const rest = remainingResults.filter(p => p?.[resultsKey]?.length).flatMap(q => q[resultsKey])
