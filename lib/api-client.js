...
export default function api(httpMethod, v2Api = false, v1Format = false) {
  let baseUrl = process.env.API_URL || 'http://localhost:3000'
  if (v2Api) baseUrl = process.env.API_URL_V2 ?? 'http://localhost:3001'
  ...
  return (path, data = {}, options = {}) => {
    ...
    let methodOptions
    if (httpMethod === 'GET') {
      const isIE11 = (typeof window !== 'undefined') && !!window.MSInputMethodContext && !!document.documentMode // client and ie11
      switch (options.cachePolicy) {
        case 'no-ie':
          // eslint-disable-next-line no-unused-expressions
          isIE11
            ? query = buildQueryString({ ...data, _: Date.now() })
            : query = buildQueryString(data)
          break
        case 'no-cache':
          // eslint-disable-next-line no-unused-expressions
          isIE11
            ? query = buildQueryString({ ...data, cache: false, _: Date.now() })
    ...
    if (v2Api && !v1Format) query = query ? `${query}&version=2` : '?version=2'
...
api.getV2 = api('GET', true)
api.getV2AsV1 = api('GET', true, true)
api.postV2 = api('POST', true)
api.putV2 = api('PUT', true)
api.deleteV2 = api('DELETE', true)
