...
const request = (httpMethod) => {
  ...
  return (path, data = {}, options = {}) => {
    ...
    return fetchFn(baseUrl + requestPath + query, { ...defaultOptions, credentials, ...methodOptions })
      .then(checkStatus)
      .then((r) => r.json())
      .catch((err) => {
        if (
...
const getInvitationById = async (invitationId, accessToken, queryParam1, queryParam2) => {
  const invitationObj = await getById('invitations', invitationId, accessToken, 1, queryParam1)
  if (invitationObj) {
    return invitationObj
  }

  return getById('invitations', invitationId, accessToken, 2, queryParam2 ?? queryParam1)
}

const getNoteById = async (noteId, accessToken, queryParam1, queryParam2) => {
  const noteObj = await getById('notes', noteId, accessToken, 1, queryParam1)
  if (noteObj) {
    return noteObj
  }

  return getById('notes', noteId, accessToken, 2, queryParam2 ?? queryParam1)
}

const getGroupById = async (groupId, accessToken, queryParam1, queryParam2) => {
  const groupObj = await getById('groups', groupId, accessToken, 1, queryParam1)
  if (groupObj) {
    return groupObj
  }

  return getById('groups', groupId, accessToken, 2, queryParam2 ?? queryParam1)
}
...
const getCombined = async (path, data1 = {}, data2 = null, options = {}) => {
  ...
  let api2Err
  if (apiRes2.status && apiRes2.status !== 200) {
    const errDetails = {
      name: apiRes2.name,
