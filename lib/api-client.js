...
const checkStatus = async (response, dispatchUser, originalRequest) => {
  if (response.ok) {
    return response
  }

  const errorBody = await response.json()
  console.log('checkStatus errorBody', errorBody)
  console.log('checkStatus originalRequest', originalRequest)
  if (errorBody.status === 401 && originalRequest) {
    // client refresh token
    console.log('api client refresh token')
    ...
    console.log('api client refresh token data is', data)
    if (data.name === 'TokenExpiredError' || data.name === 'MissingTokenError') {
      console.log('api client refresh token failed')
      dispatchUser?.({ user: null, token: null })
  ...
  console.log('checkStatus errorBody.message', errorBody.message)
...
const request = (httpMethod) => {
  ...
  return (path, data = {}, options = {}) => {
    ...
    //   defaultHeaders.Cookie = `${process.env.REFRESH_TOKEN_NAME}=${options.refreshToken}`
    // }
    ...
    return fetchFn(baseUrl + requestPath + query, fullOptions)
      .then((response) =>
        checkStatus(response, options.dispatchUser, {
