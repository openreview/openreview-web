...
import {convertNotesToV2} from './utils'
...
const getCombined = async (path, data = {}, options = {}) => {
  if (!process.env.API_V2_URL) {
    return get(path, data.queryParamV1, options)
  }

  const [apiRes1, apiRes2] = await Promise.all([
    get(path, data.queryParamV1, options),
    get(path, data.queryParamV2, { ...options, version: 2 }),
  ])
  ...
  const sortFn = sortOptions[data.sort || ''] ?? sortOptions['cdate:asc']

  if(options.useV2NoteFormat){
    // conversion works for note only
    return {
      [resultsKey]: convertNotesToV2(results1).concat(results2).sort(sortFn),
