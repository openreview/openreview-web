...
export function filterHasReplyTo(invitation, apiVersion) {
  ...
  if (apiVersion === 2) {
    return (
      ...
      invitation.edit?.note?.replyto?.param?.withForum ||
      typeof invitation.edit?.note?.id === 'string'
    )
  }

  return invitation.reply.replyto || invitation.reply.referent
}
...
export function getErrorFieldName(errorPath) {
  if (errorPath === 'signatures') return 'editSignatureInputValues'
  if (errorPath === 'note/signatures') return 'noteSignatureInputValues'
  if (errorPath === 'readers') return 'editReaderValues'
  if (errorPath === 'note/readers') return 'noteReaderValues'
  if (errorPath === 'note/license') return 'noteLicenseValue'
  if (errorPath === 'note/pdate') return 'notePDateValue'

  if (!errorPath.includes('/')) return errorPath
  const lastToken = errorPath.split('/').pop()
  if (lastToken === 'value') {
    return errorPath.split('/').slice(-2, -1)[0]
  }
  if (!Number.isNaN(Number(lastToken))) {
    return errorPath.split('/').slice(-3, -2)[0]
  }
  return lastToken
}
 ...
 * @param {boolean} castToArray - Indicates whether to cast the value to an array or string.
 * @returns {string|string[]|undefined} - The converted value.
 */
export function convertToStringArray(valueToCast, castToArray) {
  if (isNil(valueToCast) || (typeof valueToCast !== 'string' && !Array.isArray(valueToCast)))
    return undefined
  if (castToArray) {
