 ...
 * @returns object
 */
export async function parseComponentCode(entity, domain, user, args) {
  ...
  for (let i = 0; i < tokens.length; i += 1) {
    ...
    if (t === 'A') {
      if (`A${tokens[i + 1]}${tokens[i + 2]}` === 'AND') {
        ...
        currentOperand ? (currentOperand += t) : (currentOperand = t) // oxlint-disable-line no-unused-expressions
        continue
      }
    }
    if (t === 'O') {
...
export const evaluateOperator = (operator, propertyValue, targetValue) => {
  ...
  if (Array.isArray(propertyValue) && propertyValue.some((p) => p.type === 'profile')) {
    propertyValue = [
...
export function getErrorFieldName(errorPath) {
  ...
  if (errorPath === 'note/cdate') return 'noteCDateValue'
  if (errorPath === 'note/mdate') return 'noteMDateValue'

  if (!errorPath.includes('/')) return errorPath
  const lastToken = errorPath.split('/').pop()
  if (lastToken === 'value') {
    const fieldName = errorPath.split('/').slice(-2, -1)[0]
    return errorPath.startsWith('note') ? fieldName : `content.${fieldName}`
  }
