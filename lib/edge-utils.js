...
export function parseEdgeList(str, invitationCategory) {
  ...
  return str
    .split(';')
    .map((edgeStr) => {
      let [invitation, ...paramsStr] = edgeStr.split(',')
      paramsStr = [...paramsStr]
        .map((p, i) => {
          if (p.includes(':')) {
            //found a start
            let nextStart = paramsStr.findIndex((q, j) => j > i && q.includes(':'))
            if (nextStart === -1) nextStart = paramsStr.length
            return paramsStr.slice(i, nextStart).join(',')
          }
          return p
        })
        .filter((p) => p.includes(':'))
