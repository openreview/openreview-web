...
export function getEdgeBrowserUrl(configNoteContent) {
  ...
  if (configNoteContent.scores_specification) {
    ...
    edgeBrowserUrl = `/edges/browse?traverse=${configNoteContent.assignment_invitation},label:${assignmentLabel}`
...
export function buildProfileSearchText(item) {
  ...
  const { names, history, preferredEmail, emailsConfirmed } = item.content
  ...
  if (emailsConfirmed?.length) {
    emailsConfirmed.forEach((email) => {
      if (!searchFields.includes(email.toLowerCase())) searchFields.push(email.toLowerCase())
    })
  }
