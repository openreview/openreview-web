...
export function getEdgeBrowserUrl(configNoteContent, options) {
  ...
  if (configNoteContent.scores_specification) {
    ...
    edgeBrowserUrl = `/edges/browse?traverse=${traverseInvitation}`
      ...
      + (configNoteContent.custom_user_demand_invitation ? `;${configNoteContent.custom_user_demand_invitation},tail:ignore` : '')
...
function formatNoteContent(noteContent) {
  const formattedContent = {};
  Object.keys(noteContent).forEach((key) => {
    formattedContent[key] = noteContent[key].value
  })
...
export function formatEntityContent(entity, type) {
  switch (type) {
    case 'note':
      return {
        id: entity.id,
        forum: entity.forum.value,
        invitation: entity.invitations[0],
        number: entity.number,
        content: formatNoteContent(entity.content),
      }
...
export function buildNoteSearchText(item) {
  ...
  const { number, content, details } = item
  ...
  Object.keys(content).forEach((field) => {
    ...
    if (_.isString(content[field])) {
      searchFields.push(content[field])
    } else if (Array.isArray(content[field])) {
      searchFields.push(content[field].join('\n'))
    }
  })

  if (details && details.original) {
...
export function buildProfileSearchText(item) {
  const searchFields = [item.id]
  const {
    names, history, preferredEmail, emailsConfirmed, expertise,
  } = item.content

  if (names && names.length) {
    names.forEach(name => searchFields.push(
      [name.first, name.middle, name.last].filter(n => n).join(' '),
    ))
  }
  if (history) {
...
export function buildInvitationReplyArr(invitation, fieldName, profileId) {
  if (!(invitation && invitation.edge && invitation.edge[fieldName])) {
    return undefined
  }
...
export function translateFieldSpec(invitation, fieldName, version) {
  if (version === 2) {
    ...
    if (field['value-invitation']) {
      spec.query = {
        invitation: field['value-invitation'],
      }
    }
    if (field['member-of']) {
      spec.query = {
        invitation: field['member-of'],
      }
    }
    if (field['value-regex']) {
      spec['value-regex'] = field['value-regex']
    }
    if (field['value-radio']) {
      spec['value-radio'] = field['value-radio']
    }
    if (field['value-dropdown']) {
      spec['value-dropdown'] = field['value-dropdown']
    }
    return spec
  }
