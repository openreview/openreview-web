 ...
 */
export function noteContentDiffV2(leftEdit, rightEdit) {
  // eslint-disable-next-line max-len
  const allContentKeys = Array.from(new Set([...Object.keys(leftEdit.note.content), ...Object.keys(rightEdit.note.content)]))
    .filter(key => !key.startsWith('_') && key !== 'pdf' && key !== 'paperhash')

  return allContentKeys.reduce((result, key) => {
    if (!isEqual(leftEdit.note.content?.[key]?.value, rightEdit.note.content?.[key]?.value)) {
      const value1 = isNil(leftEdit.note.content?.[key]?.value) ? null : leftEdit.note.content?.[key]?.value
      const value2 = isNil(rightEdit.note.content?.[key]?.value) ? null : rightEdit.note.content?.[key]?.value
...
/**
 * merge v1 and v2 notes
 * remove duplicated results and convert v1 structure to v2
 */
export function apiV2MergeNotes(notesV1, notesV2, sortProp) {
  // eslint-disable-next-line no-use-before-define
  const v1NotesInV2Format = convertNotesToV2(notesV1, true)
  let combinedNotes = v1NotesInV2Format.concat(notesV2)
  if (sortProp) {
    combinedNotes = combinedNotes.sort((a, b) => b?.[sortProp] - a?.[sortProp])
  }
  return combinedNotes
}

/**
 * convert notes in v1 format to v2 format
 */
export function convertNotesToV2(notesV1, addVersion = false) {
  // eslint-disable-next-line no-use-before-define
  return notesV1.map(p => (convertSingleNoteToV2(p, addVersion)))
}

/**
 * convert single note in v1 format to v2 format
 */
export function convertSingleNoteToV2(noteV1, addVersion = false) {
  return {
    ...noteV1,
    content: Object.entries(noteV1.content).reduce((acc, v) => { acc[v[0]] = { value: v[1] }; return acc }, {}),
    invitations: [noteV1.invitation],
    invitation: undefined,
    ...(addVersion && { version: 'v1' }),
  }
}
