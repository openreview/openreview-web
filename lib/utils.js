...
export function editNoteContentDiff(leftEdit, rightEdit) {
  ...
  const diff = allContentKeys.reduce((result, key) => {
    const leftValue = leftEdit.note.content?.[key]?.value
    const rightValue = rightEdit.note.content?.[key]?.value
    if (!isEqual(leftValue, rightValue)) {
      const value1 = isNil(leftValue) ? null : leftValue
      const value2 = isNil(rightValue) ? null : rightValue
