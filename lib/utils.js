...
export function getRejectionReasons(profileStateInvitation, currentInstitutionName) {
  const moderationLabels =
    profileStateInvitation?.edit?.content?.labels?.value?.param?.enum ?? []
  const issuedByCurrentInstitution = currentInstitutionName
    ? `issued by ${currentInstitutionName} `
    : ''
  ...
  return moderationLabels
    ...
    .map((p) => ({
      value: p.value,
      label: p.value,
      rejectionText: p.description.replaceAll(
        '{{issuedByCurrentInstitution}}',
        issuedByCurrentInstitution
      ),
    }))
}
