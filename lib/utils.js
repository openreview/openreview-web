 ...
 */
export function prettyList(idList, style = 'long', type = 'conjunction') {
...
export function getConferenceName(bibtex) {
  ...
  return bibtex.substring(bibtex.indexOf('{', startPos) + 1, bibtex.indexOf('}', startPos))
}
...
export function forumDate(
  ...
  const mdateSettings = {
    ...
    month: 'short',
    year: 'numeric',
    ...(withTime && { hour: 'numeric', minute: 'numeric', timeZoneName: 'long' }),
  }
  const cdateSettings = {
    day: '2-digit',
    month: 'short',
  ...
  const cdateObj = hasYear ? new Date(createdYear) : new Date(cdate)
...
export function cloneAssignmentConfigNote(note) {
  ...
  const clone = omit(cloneDeep(note), [
    'id',
    'cdate',
    'forum',
    'number',
...
export function formatTasksData(
  ...
  const formatInvitation = (invitation) => {
    ...
    if (inv.noteInvitation) {
      ...
      if (completionCount && repliedCount && repliedCount >= completionCount) {
        ...
        if (inv.id.endsWith('Recommendation') && inv.apiVersion === 1) {
  ...
  const temporaryMarkExpertiseCompleted = (invitationsGroup) => {
    const profileConfirmationInv = invitationsGroup.invitations.find(
      (inv) => inv.id.endsWith('Profile_Confirmation') || inv.id.endsWith('Registration')
    )
    const expertiseInv = invitationsGroup.invitations.find((inv) =>
      inv.id.endsWith('Expertise_Selection')
    )
...
export function getDefaultTimezone() {
  ...
  const userOffsetStr =
    offsetMinutes === 0
      ? '(GMT)'
      : `(GMT ${offsetMinutes > 0 ? '-' : '+'}${Math.abs(Math.floor(offsetMinutes / 60))}:${
          offsetMinutes % 60 ? offsetMinutes % 60 : '00'
        })`
...
export function getNameString(nameObj) {
  ...
  return nameObj.middle
    ? `${nameObj.first} ${nameObj.middle} ${nameObj.last}`
 ...
 */
export function prettyTasksInvitationId(invitationId) {
  const entities = [
    'Reviewers',
    'Authors',
    'Area_Chairs',
 ...
 * @returns object
 */
export function constructRecruitmentResponseNote(invitation, content, existingNote) {
 ...
 * @returns void
 */
export function saveTextField(user, invitationId, noteId, fieldName, content) {
  const userIdForKey = !user?.id || user.id.startsWith('guest_') ? 'guest' : user.id
  const uniqueKey = `${[userIdForKey, noteId ?? 'null', invitationId ?? 'null'].join(
    '|'
  )}|${fieldName}`
 ...
 * @returns {string}
 */
export function getNotePdfUrl(note, isReference) {
  if (!note.content) {
    return ''
  }
  ...
  const urlPath = isReference
    ...
    : '/pdf'
  const pdfValue = note.version === 2 ? note.content.pdf?.value : note.content.pdf
  return pdfValue?.startsWith('/pdf') ? `${urlPath}?id=${note.id}` : pdfValue
}
 ...
 * @returns number
 */
export function getNumberFromGroup(groupId, submissionName = 'Paper', asInt = true) {
  const paper = groupId.split('/').find((p) => p.indexOf(submissionName) === 0)
  if (asInt) return paper ? parseInt(paper.substring(submissionName.length), 10) : null
