...
export function formatTasksData(
  ...
  const noteHasAllRequiredFields = (invitation, note) => {
    const requiredFields = Object.entries(invitation?.edit?.note?.content ?? {}).flatMap(
      ([field, fieldDescription]) => {
        if (fieldDescription.value.param.optional === true) return []
        return field
      }
    )

    return requiredFields.every((field) => !isEmpty(note.content?.[field]?.value))
  }
  ...
  const formatInvitation = (invitation) => {
    ...
    if (inv.noteInvitation) {
      if (
        inv.details.repliedNotes?.length > 0 &&
        ...
        sortedEdits.forEach((edit) => {
          if (!edit.note || !noteHasAllRequiredFields(invitation, edit.note)) return
          if (edit.note.ddate) {
