...
export function getTokenObjects(docArray, searchTerm) {
  ...
  const tokens = [
    ...new Set(
      contentArray
        .map((contentObj) => {
          return Object.values(contentObj.content ?? {})
            ?.flatMap((val) => {
              console.log('val', val)
...
export function getTitleObjects(docArray, searchTerm) {
  const termRegex = new RegExp(`${_.escapeRegExp(searchTerm)}.*`, 'i')
  return docArray
    .map((docObj) => {
    ...
    .filter(
      (titleObj) =>
        (!_.isEmpty(titleObj.value) && titleObj.value.match(termRegex)) ||
        (!_.isEmpty(titleObj.authors) && contentToString(titleObj.authors).match(termRegex))
    )
}
