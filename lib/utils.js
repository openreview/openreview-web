...
export function formatTasksData(
  ...
  const formatInvitation = (invitation) => {
    ...
    const group = inv.domain ?? getGroupId(inv.id)
...
export function getNotePdfUrl(note, isReference, isV2Note) {
  ...
  const urlPath = isReference ? `${isV2Note ? '/notes/edits/pdf' : '/references/pdf'}` : '/pdf'
...
export function getProfileName(profile) {
  ...
  return name ? `${name.first} ${name.last}` : prettyId(profile.id)
}
 ...
 * @param {string} [pengingText='pending']
 * @param {string} [errorText='error']
 * @returns string
 */
export function getTabCountMessage(
  pendingCount,
  errorCount,
  pengingText = 'pending',
  errorText = 'error'
  ...
  if (pendingCount === 0 && errorCount !== 0)
    return inflect(errorCount, errorText, `${errorText}s`, true)
  if (pendingCount !== 0 && errorCount === 0) return pendingCount
  return `${pendingCount} ${pengingText},${inflect(
    errorCount,
    errorText,
    `${errorText}s`,
    true
...
export function useNewNoteEditor(invitationDomain) {
  if (!invitationDomain) return false

  const noteEditorVenues = process.env.NEW_NOTE_EDITOR_VENUES
  return noteEditorVenues.some((p) => p === 'all' || p === invitationDomain)
}
