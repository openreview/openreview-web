...
export function apiV2MergeNotes(notesV1, notesV2) {
  // when v1 and v2 have no duplicates should use commented code
  // const v2NoteIds = notesV2.map(p => p.id)
  // const v1OnlyNotes = notesV1.filter(p => !v2NoteIds.includes(p.id))
  // const v1OnlyNotesInV2 = v1OnlyNotes.map(p => ({
  //   ...p,
  //   content: Object.entries(p.content).reduce((acc, v) => { acc[v[0]] = { value: v[1] }; return acc }, {}),
  //   invitations: [p.invitation],
  //   invitation: undefined,
  //   version: 'v1',
  // }))
  // return v1OnlyNotesInV2.concat(notesV2)
  const v1NoteIds = notesV1.map(p => p.id)
  const v2OnlyNotes = notesV2.filter(p => !v1NoteIds.includes(p.id)).map(q => ({ ...q, version: 'v2' }))
  const v1NotesInV2 = notesV1.map(p => ({
    ...p,
    content: Object.entries(p.content).reduce((acc, v) => { acc[v[0]] = { value: v[1] }; return acc }, {}),
    invitations: [p.invitation],
    invitation: undefined,
  ...
  return v1NotesInV2.concat(v2OnlyNotes)
}

/**
 * convert notes in v1 format to v2 format
 */
export function convertNotesToV2(notesV1) {
  // eslint-disable-next-line no-use-before-define
  return notesV1.map(p => (convertSingleNoteToV2(p)))
}

/**
 * convert single note in v1 format to v2 format
 */
export function convertSingleNoteToV2(noteV1) {
  return {
    ...noteV1,
