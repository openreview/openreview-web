...
export function editNoteContentDiff(leftEdit, rightEdit) {
  ...
  const diff = allContentKeys.reduce((result, key) => {
  ...
  // eslint-disable-next-line max-len
  if (!isEqual(leftEdit.invitations[0], rightEdit.invitations[0])) diff.invitation = { left: leftEdit.invitations[0], right: rightEdit.invitations[0] }
  return diff
}
