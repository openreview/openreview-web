...
/**
 * convert dot notation assignment statement to object
 *
 * @param {string} prefilledValue - e.g. edit.note.comment.value=test comment
 * @returns object
 */
export function stringToObject(prefilledValue) {
  const obj = {}
  const tokens = prefilledValue?.split('.')
  ...
  const [key, value] = tokens[tokens.length - 1].split('=')
