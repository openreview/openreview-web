...
export function getDefaultTimezone() {
  ...
  const userOffsetStr =
    offsetMinutes === 0
      ? '(GMT)'
      : `(GMT ${offsetMinutes > 0 ? '-' : '+'}${Math.floor(Math.abs(offsetMinutes / 60))}:${offsetMinutes % 60 ? Math.abs(offsetMinutes % 60) : '00'
      })`
  ...
  const rejectionReasons = [
    {
      ...
      rejectionText: `Please add and confirm an institutional email ${currentInstitutionName ? `issued by ${currentInstitutionName} ` : ''
        }to your profile. Please make sure the verification token is entered and verified.\n\nIf your affiliation ${currentInstitutionName ? `issued by ${currentInstitutionName} ` : ''
