...
export function editNoteContentDiff(leftEdit, rightEdit) {
  ...
  const allContentKeys = Array.from(new Set([...getAllKeys(leftEdit ?? {}), ...getAllKeys(rightEdit ?? {})])).map(p => p.toLowerCase())
    .filter(key => !key.startsWith('_') && key !== 'note.content.pdf' && key !== 'note.content.paperhash')
  ...
  const diff = allContentKeys.reduce((result, key) => {
    ...
    if (!isEqual(leftValue, rightValue)) {
      const value1 = isNil(leftValue) ? null : formatDateValue(key, leftValue)
      const value2 = isNil(rightValue) ? null : formatDateValue(key, rightValue)
