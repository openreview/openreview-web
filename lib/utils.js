...
export function forumDate(
  ...
  if (
    ...
    const mdateFormatted = mdate ? new Date(mdate).toLocaleDateString('en-GB', mdateSettings) : ''

    if (pdate) {
      const pdateFormatted = new Date(pdate).toLocaleDateString('en-GB', mdateSettings)
      const secondaryDate = mdate ? `Last Modified: ${mdateFormatted}` : `Uploaded: ${cdateFormatted}`
