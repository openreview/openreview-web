...
export function getTitleObjects(docArray, searchTerm) {
  ...
  return docArray
    .map((docObj) => {
      ...
      const title =
        docObj.version === 2 ? contentObj.title?.value : contentObj.title
      const authors =
        docObj.version === 2 ? contentObj.authors?.value : contentObj.authors
      return {
        value: _.isEmpty(title) ? '' : title,
        forum: docObj?.forum ?? '',
        id: docObj.id,
        label: _.isEmpty(title) ? '' : emphasize(title, searchTerm),
        subtitle: _.isEmpty(authors)
          ? ''
          : emphasize(contentToString(authors), searchTerm),
    ...
    .filter(
      (titleObj) =>
        ...
        (!_.isEmpty(titleObj.authors) &&
          contentToString(titleObj.authors).match(termRegex))
    )
}
...
const truncate = (label, boldTerm) => {
  ...
  if (m && m.index && label.length > maxCharLength) {
    ...
    while ((prefix + boldTerm + suffix).length < maxCharLength) {
      prefix = `${ellipses}${beforeWords
        .slice(t, beforeWords.length - 1)
        .join(' ')} `
      t = t - 1
    }
