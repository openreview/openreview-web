...
export function forumDate(
  ...
  if (pdate) {
    const pdateFormatted = new Date(pdate).toLocaleDateString(
      'en-GB',
...
/**
 * @param {string} fullname - the name for comparison
 * @returns {string}
 */
export function normalizeName(fullname) {
  return fullname
    .normalize('NFKD')
    .replace(/[\u0300-\u036f]/g, '') // remove ́ generated NFKD normalization of é
    .replace(/[^\p{L}\p{N}]+/gu, ' ')
    .trim()
    .replace(/\s+/g, ' ')
}
...
export function getTitleObjects(docArray, searchTerm) {
  ...
  return docArray
    .map((docObj) => {
      ...
      const authors = getNoteAuthors(docObj, docObj.version === 2)
      return {
...
export function getRejectionReasons(currentInstitutionName) {
  const rejectionReasons = [
    ...
    },
    {
      ...
      rejectionText:
        'Please confirm the institutional email in your profile by clicking the "Confirm" button next to the email and enter the verification token received.',
    },
    {
      value: 'invalidDBLP',
    ...
    },
    {
      ...
      rejectionText:
        'The name in your profile does not match the name listed in your homepage or is invalid.',
    },
    {
      value: 'invalidORCID',
      label: 'ORCID profile is incomplete',
      rejectionText:
        "The ORCID profile you've provided as a homepage is empty or does not match the Career & Education history you've provided.",
    },
    {
      value: 'requestVouch',
    ...
    },
    {
      ...
      label: 'Profiles cannot represent an organization',
      rejectionText:
        'Profiles can only represent an individual, we do not allow profiles to be created for an organization.',
    },
    {
      value: 'incompleteHistory',
      label: 'Education/career history is incomplete',
      rejectionText:
        'Please complete your full education and career history in your profile, not just your current position.\n\nA complete history helps us verify your identity and is required for profile activation.',
    },
    {
      value: 'documentVerification',
      label: 'Documentation / ID requests',
      rejectionText:
        "To proceed with activation and verify your affiliation, please send a document from each of the following categories to info@openreview.net:\n\nA photo ID (government ID, passport, or driver's license) to verify your identity.\n\nA certificate or official document proving your affiliation with the institution listed in your profile (e.g. enrollment letter, employee ID, or student ID), or a link to your institutional directory page (the email you sign up with must match what is listed on the page).\n\nWe handle all documents in accordance with our privacy policy and delete them after verification is complete.\n\nOnce we have reviewed your documents, we will activate your profile manually. Please do not resubmit your profile to moderation in the meantime, as this will result in your profile being rejected again.",
    },
    {
    ...
    {
      value: 'documentVerificationLink',
      label: 'Document Verification Form Upload Link',
      rejectionText:
        'Please use the following link to upload the requested identity verification documents:\n\n{{documentVerificationLink}}\n\nOnce you have uploaded all of the requested documents, use the activation link in this email to update and submit your profile information. Please do not submit your profile before completing the upload, as it may be rejected again.',
    },
    {
      value: 'underageConsentLink',
      label: 'Underage Consent Form Upload Link',
      rejectionText:
        'Please use the following link to upload the completed consent form:\n\n{{underageConsentLink}}\n\nOnce you have uploaded the consent form, use the activation link in this email to update and submit your profile information. Please do not submit your profile before completing the upload, as it may be rejected again.',
    },
  ]
...
/**
 * return authorids from a note, handling v1 note, v2 note array and object format
 *
 * @param {object} note - the note to get authorids from
 * @param {boolean} isV2Note - whether the note is a v2 note
 * @returns Array
 */
export function getNoteAuthorIds(note, isV2Note) {
...
/**
 * return author names from a note, handling v1 note, v2 note array and object format
 *
 * @param {object} note - the note to get author names from
 * @param {boolean} isV2Note - whether the note is a v2 note
 * @returns Array
 */
...
export function sanitizeRedirectUrl(redirectUrl) {
  try {
    ...
    if (allowedRedirectPaths.has(url.pathname))
