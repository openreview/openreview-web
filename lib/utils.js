...
export function prettyId(id, onlyLast) {
  ...
  if (!id) {
    ...
    if (idString.includes('${')) {
      ...
      idString = idString.replace(match, newMatch)
    }
    ...
    if (inv.noteInvitation) {
      ...
      if (isEmpty(inv.details.replytoNote)) {
        ...
        inv.details.replytoNote = {
          forum: inv.apiVersion === 1 ? inv.reply.forum : inv.edit.note.forum,
        }
      }
...
export function getProfileName(profile) {
  ...
  return name ? prettyId(profile.id) : `${name.first} ${name.last}`
}
