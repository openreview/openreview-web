...
export function prettyList(idList, style = 'long', type = 'conjunction', isRawId = true) {
  ...
  const prettyArr = isRawId ? idList.map((id) => prettyId(id)) : idList
 ...
 * @returns the key
 */
export function getAutoStorageKey(user, invitationId, noteId, fieldName) {
  const userIdForKey = !user?.id || user.id.startsWith('guest_') ? 'guest' : user.id
  return `${[userIdForKey, noteId ?? 'null', invitationId ?? 'null'].join('|')}|${fieldName}`
}
