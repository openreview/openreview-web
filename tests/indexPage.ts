...
test('show active venues', async t => {
  await t.navigateTo(`http://localhost:${process.env.NEXT_PORT}`)
    ...
    .expect(Selector('#open-venues').child.length).eql(2)
    .expect(Selector('#open-venues').find('a').nth(0).textContent).eql(conferenceGroupId.replace(/\//g,' '))
    .expect(Selector('#open-venues').find('a').nth(1).textContent).eql(`Another${conferenceGroupId}`.replace(/\//g,' '))
    .expect(Selector('span').withText('Due').count).eql(2)
