...
test('show active venues', async (t) => {
  ...
  await t
    // Active venues
    .expect(activeVenues.find('li').count).eql(3)
    ...
    .expect(openVenues.find('li').count).eql(3)
    ...
    .expect(openVenues.find('p').withText('Due').count).eql(3)

    // All venues
    .expect(allVenues.find('li').count).eql(3)
    .expect(allVenues.find('a').nth(0).textContent).eql(`Another${baseGroupId}`)
    .expect(allVenues.find('a').nth(1).textContent).eql('ICLR')
    .expect(allVenues.find('a').nth(2).textContent).eql(baseGroupId)
})
