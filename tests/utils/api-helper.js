...
export async function setupRegister(superUserToken) {
  ...
  await createProfile(inActiveUserNoPassword.first, inActiveUserNoPassword.last, inActiveUserNoPassword.email, inActiveUserNoPassword.tildeId, superUserToken)
  // eslint-disable-next-line max-len
  await createEmptyProfile(inActiveUserNoPasswordNoEmail.first, inActiveUserNoPasswordNoEmail.last, inActiveUserNoPasswordNoEmail.tildeId, superUserToken)
}
...
export function addMembersToGroup(groupId, membersList, userToken) {
  return api.put('/groups/members', { id: groupId, members: membersList }, { accessToken: userToken })
}
