...
test('add alternate email', async (t) => {
  ...
  let consoleInfo = await t.getBrowserConsoleMessages()
  console.log(consoleInfo)
  await t.expect(Selector('h4').withText('Emails').exists)
    .ok()
    ...
    .click(Selector('#emails_table').child('tbody').child('tr').nth(1)
      ...
      .nth(1)
      .child('button'))
  consoleInfo = await t.getBrowserConsoleMessages()
  console.log(consoleInfo)
  await t.expect(Selector('#flash-message-container').exists)
    .ok()
    .expect(Selector('span').withAttribute('class', 'important_message').innerText)
