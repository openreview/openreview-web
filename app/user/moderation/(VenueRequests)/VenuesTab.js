    ...
    try {
      const notesResult = await api.getCombined(
        ...
        },
        {
          ...
          sort: 'cdate',
          details: 'replies',
          select: `id,forum,parentInvitations,signatures,cdate,content.state,content.abbreviated_venue_name,content.venue_id,content.status,details.replies[*].id,details.replies[*].replyto,details.replies[*].content.comment,details.replies[*].invitations,details.replies[*].signatures,details.replies[*].cdate,details.replies[*].cdate`,
        },
        { includeVersion: true }
      )
