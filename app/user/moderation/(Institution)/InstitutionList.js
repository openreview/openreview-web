...
export default function InstitutionList({ accessToken, institutionsP }) {
  const intitialInstitutions = use(institutionsP)
  const [institutions, setInstitutions] = useState(intitialInstitutions)
  const [institutionsToShow, setInstitutionsToShow] = useState(institutions.slice(0, pageSize))
  const [page, setPage] = useState(1)
  const [institutionToEdit, setInstitutionToEdit] = useState(null)
  const [countryOptions, setCountryOptions] = useState([])

  const reloadInstitutionsDomains = async (noCache) => {
    try {
  ...
  return (
    <div>
      <InstituitonSearchForm
        ...
        setPage={setPage}
        reloadInstitutionsDomains={reloadInstitutionsDomains}
