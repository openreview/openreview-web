      ...
      <div id={collapseId} className="collapse">
        <ul className="list-unstyled note-content-list">
          {Object.keys(content).map((fieldName) => {
            if (omittedContentFields.includes(fieldName) || fieldName.startsWith('_') || !content[fieldName]) {
              return null
            }
            ...
            return (
              <li key={fieldName}>
                ...
                {' '}
                <span className="note-content-value">{prettyContentValue(content[fieldName])}</span>
              </li>
            )
          })}
        </ul>
