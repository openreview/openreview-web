...
const BinStat = ({ id, stats, edgeBrowserUrlParams }) => {
  ...
  const draw = () => {
    if (!stats) return
    const margin = {
      top: 40, right: 10, bottom: 40, left: 50,
    }
    ...
    const xAxisBarLabelFontStyle = stats.tag === 'continuous'
      ? '.50rem sans-serif'
      : '.75rem sans-serif'
    const min = stats.tag === 'continuous' && Math.min(stats.data) < stats.min ? 2 * floor(Math.min(stats.data), 1) : stats.min
    const xmax = stats.tag === 'discrete' ? Math.max(stats.max, 10) + 0.5 : stats.max
    const xmin = stats.tag === 'discrete' ? min - 0.5 : min
    const x = scaleLinear().domain([xmin, xmax]).rangeRound([0, width - 10])
    ...
    const groups = svg.selectAll('rect').data(bins).enter().append('g')
    ...
    const rect = groups.append('rect')
      ...
      .attr('width', d => (x(d.x1) - x(d.x0) - barspace < 0 ? 0 : x(d.x1) - x(d.x0) - barspace))
    ...
    if (stats.interactiveData) {
      ...
      rect.on('click', (e, d) => {
        ...
        if (stats.tag === 'discrete') {
          ...
          dataList = stats.interactiveData.filter(p => p.num === num).map(q => q.data).sort()
        } else {
          dataList = [...new Set(stats.interactiveData.filter(p => p.num >= d.x0 && p.num < d.x1).map(q => q.data))]
        }
        const localStorageKey = stats.tag === 'discrete' ? `${id}-x-${d[0]}` : `${id}-x-${d.x0}-to-${d.x1}`
        window.localStorage.setItem(localStorageKey, JSON.stringify({
          type: stats.type,
          data: dataList,
        }))
        const edgeBrowserUrl = `/edges/browse?\
start=staticList,${stats.type === 'reviewer' ? 'type:tail' : 'type:head'},storageKey:${localStorageKey}\
...
&referrer=${encodeURIComponent(`[${referrerText}](/assignments/stats?id=${configNoteId})`)}`
    ...
    // #region X Axis
    const xAxisTickCount = stats.tag === 'discrete'
      ? Math.round(xmax - xmin)
      : binCount

    const xAxisTickFontSize = stats.tag === 'continuous'
      ? '0.5rem'
      : ''
