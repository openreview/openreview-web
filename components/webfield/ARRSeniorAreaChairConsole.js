...
const ARRSeniorAreaChairConsole = ({ appContext }) => {
  const {
    header,
    entity: group,
    venueId,
  ...
  const loadData = async () => {
    ...
    try {
      ...
      // #region get perPaperInvitations
      ...
      : []
      // Use note.number to map to the correct perPaperInvitation by checking ${submissionName}${note.number}/ string
      const perPaperInvitationForumsMap = new Map()
      notes.forEach((note) => {
        const noteNumber = note.number
        const notePerPaperInvitations = perPaperInvitations.filter((invitation) =>
          invitation.id.includes(`${submissionName}${noteNumber}/`)
        ).map((invitation) => ({'suffix': invitation.id.split('/-/')[1], 'forum': invitation.edit.note.forum}))
        perPaperInvitationForumsMap.set(noteNumber, notePerPaperInvitations)
      }
      )
      console.log(perPaperInvitationForumsMap)
      ...
      setSacConsoleData({
        ...
        notes: assignedNotes.flatMap((note) => {
          ...
          const perPaperInvitationForums = perPaperInvitationForumsMap.get(note.number) ?? []
...
export default ARRSeniorAreaChairConsole
