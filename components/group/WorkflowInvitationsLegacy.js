// The workflow page as it was before stages: a flat list of steps. Used for venues whose
// domain has no `workflow_stages`; the staged timeline in WorkflowInvitations.js needs it.
...
const WorkflowTasks = ({ workflowTasks, setCollapsedWorkflowInvitationIds }) => {
  ...
  return (
    <section className="workflow workflow-legacy">
      <div
...
const WorkflowInvitationsLegacy = ({ group }) => {
  ...
  return (
    <>
      {workflowGroups.size > 0 && (
        <EditorSection
          title={`Workflow Groups (${workflowGroups.size})`}
          className="workflow workflow-legacy"
        >
...
export default WorkflowInvitationsLegacy
